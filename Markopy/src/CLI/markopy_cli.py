...
parser.add_argument("input", help="Input model file. This model will be imported before starting operation.\n"
                                  + "For more information on the file structure for input, check out the wiki page.")
parser.add_argument("-o", "--output",
                    help="Output model filename. This model will be exported when done. Will be ignored for generation mode.")
parser.add_argument("-d", "--dataset",
                    help="Dataset filename to read input from for training. Will be ignored for generation mode.\n"
                    + "Dataset is occurrence of a string and the string value seperated by a seperator. For more info "
                    + "on the dataset file structure, check out the github wiki page.")
parser.add_argument("-s", "--seperator",
                    help="Seperator character to use with training data.(character between occurrence and value)\n"
                    + "For more information on dataset/corpus file structure, check out the github wiki.")
parser.add_argument("-w", "--wordlist",
                    help="Wordlist filename path to export generation results to. Will be ignored for training mode")
parser.add_argument("--min", default=6, help="Minimum length that is allowed during generation.\n"
                    + "Any string shorter than this paremeter will retry to continue instead of proceeding to "
                    + "finishing node")
parser.add_argument("--max", default=12, help="Maximum length that is allowed during generation.\n"
                    +"Any string that does reaches this length are cut off irregardless to their position on the model.")
parser.add_argument("-n", "--count", help="Number of lines to generate. Ignored in training mode.")
parser.add_argument("-t", "--threads", default=10, help="Number of threads to use with training/generation.\n"
                    +"This many OS threads will be created for training/generation functions")
parser.add_argument("-v", "--verbosity", action="count", help="Output verbosity.\n"
                    + "Set verbosity to 1: -v\n"
                    + "Set verbosity to 3: -vvv\n"
                    + "Print pretty much everything, including caller functions: -vvvvvvvvvvvvvvv")
parser.add_argument("-b", "--bulk", action="store_true",
                    help="Bulk generate or bulk train every corpus/model in the folder.\n"
                    + "If working on this mode, output/input/dataset parameters should be a folder.\n"
                    + "Selected operation (generate/train) will be applied to each file in the folder, and "
...
def cli_train(model, dataset, seperator, output, output_forced=False, bulk=False):
    if not (dataset and seperator and (output or not output_forced)):
        logging.pprint(
            f"Training mode requires -d/--dataset{', -o/--output' if output_forced else ''} and -s/--seperator parameters. Exiting.")
        exit(2)
...
if (args.bulk):
    ...
    if (args.mode.lower() == "train"):
        if (os.path.isdir(args.output) and not os.path.isfile(args.output)) and (
                os.path.isdir(args.dataset) and not os.path.isfile(args.dataset)):
            ...
            for corpus in corpus_list:
                ...
                cli_train(model, f"{args.dataset}/{corpus}", args.seperator,
                          f"{args.output}/{corpus}.{model_extension}", output_forced=True, bulk=True)
        else:
            logging.pprint("In bulk training, output and dataset should be a directory.")
            exit(1)

    elif (args.mode.lower() == "generate"):
        if (os.path.isdir(args.wordlist) and not os.path.isfile(args.wordlist)) and (
                os.path.isdir(args.input) and not os.path.isfile(args.input)):
